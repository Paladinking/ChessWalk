...
public class AStar {
    ...
    public void setBlocks(Tile[][] blocksArray, Entity e) {
        for (int i = 0; i < blocksArray.length; i++) {
            for (int j = 0; j < blocksArray[0].length; j++) {
                if(!blocksArray[j][i].selected(e)){
                    setBlock(i,j);
                }
            }
        }
    ...
    public ArrayList<Point> topPoints(List<Node> l){
        ArrayList<Point> p = new ArrayList<>();
        for (Node n:l) {
            p.add(new Point(n.getCol(),n.getRow()));
        }
        return p;
    }
    public ArrayList<Point> findPathPoint(){
        return topPoints(findPath());
    }
